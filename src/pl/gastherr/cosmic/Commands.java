	...
	public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("pinfo")){
			...
			if (args.length == 0 || args.length == 1){
				...
				String name = p.getName();
				Gracz g = PlayerJoin.listaGraczy.get(p.getUniqueId().toString());
				double kasa = g.getStaty().getKasa();
				String kasaFormat = changeMoneyFormat((int) kasa);
				
				int pktPost = g.getPktPostepu();
				
				int frakcjaInstor = g.getStaty().getUzyciaInstor();
				int frakcjaForter = g.getStaty().getUzyciaForter();
				int frakcjaProter = g.getStaty().getUzyciaProter();
				int frakcjaPread = g.getStaty().getUzyciaPread();
				...
				float totalExp = g.getStaty().getExp();
				float expToLvl = 0;
				expToLvl = Math.round(totalExp*100);
				String show = ""+expToLvl+"%";
				String frakcja = g.getFrakcja();
				ChatColor kolor = ChatColor.GOLD;
				...
				if (args.length == 1){
					...
					if (args[0].equalsIgnoreCase("2")){
						...
						s.sendMessage(ChatColor.GRAY+"Smierci: "+kolor+smierci);
						s.sendMessage(ChatColor.GRAY+"KDR (zabojstwa / smierci): "+kolor+kdr);
						s.sendMessage(ChatColor.GRAY+"Aby zobaczyc kolejna strone wpisz "+kolor+"/pinfo 3");
					}
					if (args[0].equalsIgnoreCase("3")){
			...
			if (cmd.getName().equalsIgnoreCase("spawn")){
				...
				final Gracz g = PlayerJoin.listaGraczy.get(p.getUniqueId().toString());
				boolean isTeleporting = g.getIsTeleporting();
				final ItemStack nic = new ItemStack(Material.AIR);
				
				if (isTeleporting == true){
					p.sendMessage(ChatColor.RED+"Nie mozesz uzyc tej komendy teraz!");
					return false;
				}
					...
					final boolean f = g.getWalka().getIsFighting();
						...
						public void run() {
							if (g.getIsTeleporting()){
								...
								if (f){
									...
									g.setFrakcja("Brak");
									g.getWalka().setMechList(null);
									g.getWalka().setIsFighting(false);
								}
							}	
						}
					}, 20*5);
			...
			if(cmd.getName().equalsIgnoreCase("upgrade")){
				Player p = (Player) s;
				Gracz g = PlayerJoin.listaGraczy.get(p.getUniqueId().toString());
				int pkt = g.getPktPostepu();
				int iLvl = 0;
				
				if (pkt == 0){
					p.sendMessage(ChatColor.RED+"Nie masz zadnego punktu postepu, aby ulepszyc przedmiot!");
					return false;
				}
				
				if (args[0].equalsIgnoreCase("miecz")){
					g.setPktPostepu(pkt-1);
					iLvl = g.getStaty().getMieczLvl();
					g.getStaty().setMieczLvl(iLvl+1);
					s.sendMessage(ChatColor.GRAY+"Ulepszono "+ChatColor.GOLD+args[0]+ChatColor.GRAY+" do poziomu "+iLvl);
				}
					
				if (args[0].equalsIgnoreCase("helm")){
					g.setPktPostepu(pkt-1);
					iLvl = g.getStaty().getHelmLvl();
					g.getStaty().setHelmLvl(iLvl+1);
					s.sendMessage(ChatColor.GRAY+"Ulepszono "+ChatColor.GOLD+args[0]+ChatColor.GRAY+" do poziomu "+iLvl);
				}
				if (args[0].equalsIgnoreCase("klata")){
					g.setPktPostepu(pkt-1);
					iLvl = g.getStaty().getKlataLvl();
					g.getStaty().setKlataLvl(iLvl+1);
					s.sendMessage(ChatColor.GRAY+"Ulepszono "+ChatColor.GOLD+args[0]+ChatColor.GRAY+" do poziomu "+iLvl);
				}
				if (args[0].equalsIgnoreCase("spodnie")){
					g.setPktPostepu(pkt--);
					iLvl = g.getStaty().getSpodnieLvl();
					g.getStaty().setSpodnieLvl(iLvl++);
					s.sendMessage(ChatColor.GRAY+"Ulepszono "+ChatColor.GOLD+args[0]+ChatColor.GRAY+" do poziomu "+iLvl);
				}
				if (args[0].equalsIgnoreCase("buty")){
					g.setPktPostepu(pkt--);
					iLvl = g.getStaty().getButyLvl();
					g.getStaty().setButyLvl(iLvl++);
					s.sendMessage(ChatColor.GRAY+"Ulepszono "+ChatColor.GOLD+args[0]+ChatColor.GRAY+" do poziomu "+iLvl);
				}
			}
		return false;
